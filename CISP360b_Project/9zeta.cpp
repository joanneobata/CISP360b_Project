...
void bField(string msg){
	int padding = 5;
	cout << fixed << left;
	cout << setw(padding) << Br << setw(mapWidth - (padding + 1)) << msg << Br;
	cout << endl;
}
...
int newField(string msg, int k){
	if(k == 0){
		...
		cout << fixed << left;
		cout << setw((3 * padding)) << Br;			// starts a new sentence
		cout << setw(mapWidth - (4 * padding))
			<< msg;
		cout << setw(padding) << right << Br;         // ends a new sentence
		return 1;
	}
	else if (k == 1) {
		int padding = mapWidth / 10;
